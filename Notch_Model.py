...
Rule('geneSlug_binds_ICDCSL', gSlug(ic=None) + CSL(icd=ANY, gsl=None) <> gSlug(ic=1)%CSL(icd=ANY,gsl=1),kICDCSLSlugf,kICDCSLSlugr)
...
Rule('geneAscl1_bind_proteinHES1', gAscl1(ph=None)+pHES1(ga1=None) <> gAscl1(ph=1)%pHES1(ga1=1), kHes1Ascl1f,kHes1Ascl1r)
...
Rule('gLig_in_binds_to_Ngn2', gL_in(n2=None) + pNGN2(gl_in=None) <> gL_in(n2=1)%pNGN2(gl_in=1), kNgn2L_inf,kNgn2L_inr)
...
Rule('Recycle_Receptor_Endo_L_to_R', Endo_off_R_ex() + ICD(l='cyto') >> R_ex(l_in=None,ysc=None),rR_ex)#rate is v low
